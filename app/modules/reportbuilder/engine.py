...
def run_pandoc(markdown: str, resource_dir: Path) -> str:
    ...
    if result.returncode != 0:
        # Raise, don't sys.exit: this runs as a library from the web route, and
        # SystemExit is not caught by `except Exception`, so sys.exit here would
        # kill the request thread instead of returning a "build failed" message.
