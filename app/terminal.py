...
# ----------------------------------------------------------------------------
# CSRF guard  (the loopback check above is NOT one)
# ----------------------------------------------------------------------------
# The loopback guard stops other HOSTS on the network, but it does nothing
# against a cross-site request the user's OWN browser is tricked into sending:
# a malicious page can fetch() http://127.0.0.1:8090/api/term/send while
# Blackheart runs, and remote_addr/Host are still loopback. That is a drive-by
# RCE into the tmux panes. We reject cross-site requests to the state-changing
# routes below.
#   - Sec-Fetch-Site is set by all current browsers and CANNOT be overridden by
#     JavaScript, so it is the primary check.
#   - Origin is the fallback for the rare client that doesn't send Sec-Fetch-*.
#   - A non-browser client (curl, the CLI) sends neither and is not a CSRF
#     vector, so it is allowed through -- the loopback guard still applies.
_ALLOWED_ORIGINS = {
...
def _is_cross_site():
    ...
    return False  # no browser fetch-metadata -> not a browser CSRF vector


def same_origin_only(fn):
    @wraps(fn)
    def wrapper(*a, **kw):
        if _is_cross_site():
            abort(403)  # cross-site request to a command-execution endpoint
        return fn(*a, **kw)
    return wrapper
...
@same_origin_only
def api_send():
    # Require a real JSON content-type BEFORE anything else: a text/plain body
    # is a CORS "simple request" that a cross-site page can send without a
    # preflight. Refusing non-JSON closes that path (the same-origin guard
    # already blocks it; this is defence in depth).
